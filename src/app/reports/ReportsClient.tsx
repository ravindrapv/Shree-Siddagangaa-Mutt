  ...
  return (
    <div className="space-y-6">
      ...
      {/* -------------------------------------------------------------
          ...
          ------------------------------------------------------------- */}
      <PrintPortal>
        <div className="print-only a4-receipt text-black font-sans bg-white p-8">
          <div className="flex justify-between items-center border-b-2 border-black pb-4 mb-6">
            {/* Swamiji Photos */}
            <div className="flex gap-2">
              <div className="w-12 h-12 rounded-full overflow-hidden border border-black bg-white">
            ...
            <div className="text-center flex-1 mx-4">
              <h1 className="text-[17px] font-black text-black">ಶ್ರೀ ಸಿದ್ದಗಂಗಾ ಮಠ, ತುಮಕೂರು</h1>
              <h2 className="text-xs font-bold text-orange-600 mt-0.5">ಹಣಕಾಸು ವರದಿ ಮತ್ತು ಲೆಕ್ಕಪತ್ರ ಸಾರಾಂಶ</h2>
              <h3 className="text-[9px] text-gray-500 font-bold uppercase tracking-wider mt-0.5">FINANCIAL REVENUE & STAY AUDIT SUMMARY</h3>
              <p className="text-[9px] text-gray-500 font-bold mt-1" suppressHydrationWarning>ಆಡಿಟ್ ಅವಧಿ (Period): {formatDate(startDate)} to {formatDate(endDate)}</p>
            </div>

            {/* Right Meta */}
            <div className="text-right text-[8px] font-black border border-black p-1.5 bg-gray-50 rounded uppercase tracking-wider text-black">
              ಕಚೇರಿ ಪ್ರತಿ<br />OFFICE COPY
            </div>
          </div>

          <div className="my-6 grid grid-cols-2 gap-6 text-[11px] font-bold text-gray-700">
            <div className="space-y-1">
              <p suppressHydrationWarning>ವರದಿ ದಿನಾಂಕ (Report Date): {new Date().toLocaleDateString("en-IN")}</p>
              <p>ರಚಿಸಿದವರು (Generated by): System Administrator</p>
            </div>
            <div className="text-right space-y-1">
              <p>ಒಟ್ಟು ನೋಂದಣಿಗಳು (Total Stays): {filteredBookings.length}</p>
              <p className="text-xs font-black text-black">ಒಟ್ಟು ಸಂಗ್ರಹಣೆ (Total Collection): ₹ {totalCollected}.00</p>
            </div>
          </div>

          <table className="w-full border-collapse border border-black text-[10.5px] my-6 text-left">
            <thead>
              <tr className="bg-gray-100 border-b border-black text-[9px] font-bold uppercase text-black">
                <th className="p-2 border-r border-black font-extrabold">ದಿನಾಂಕ (Date)</th>
                <th className="p-2 border-r border-black font-extrabold">ರಶೀದಿ ಸಂಖ್ಯೆ (Receipt ID)</th>
                <th className="p-2 border-r border-black font-extrabold">ಪಾವತಿ ವಿವರಣೆ (Transaction Notes)</th>
                <th className="p-2 border-r border-black font-extrabold">ವಿಧಾನ (Method)</th>
                <th className="p-2 font-extrabold text-right">ಜಮಾ ಮೊತ್ತ (Amount Paid)</th>
              </tr>
            </thead>
            <tbody>
              {reportCollections.map((p) => (
                <tr key={p.id} className="border-b border-black text-black">
                  <td className="p-2 border-r border-black text-[9.5px]" suppressHydrationWarning>{formatDateTime(p.date)}</td>
                  <td className="p-2 border-r border-black font-bold font-mono">{p.receiptNo}</td>
                  <td className="p-2 border-r border-black text-[9.5px]">{p.notes || "Stay Settle Settle"}</td>
                  <td className="p-2 border-r border-black font-bold text-center">{p.method}</td>
                  <td className="p-2 text-right font-bold">₹ {p.amount}.00</td>
                </tr>
              ))}
              <tr className="bg-gray-50 border-t border-b-2 border-black font-bold">
                <td colSpan={4} className="p-2.5 text-right font-black border-r border-black text-[10px] uppercase">ಒಟ್ಟು ಜಮಾ ಮೊತ್ತ (Grand Total Collection):</td>
                <td className="p-2.5 text-right font-black text-xs text-black">₹ {totalCollected}.00</td>
              </tr>
            </tbody>
          </table>

          <div className="mt-20 flex justify-between text-[10px] font-bold">
            <div className="border-t border-black w-44 text-center pt-2">
              ಖಜಾಂಚಿ ಸಹಿ (Treasurer Signature)
            </div>
            <div className="border-t border-black w-44 text-center pt-2">
              ಅಧಿಕೃತ ಸಹಿ (Authorized Admin)
            </div>
          </div>
        </div>
      </PrintPortal>
